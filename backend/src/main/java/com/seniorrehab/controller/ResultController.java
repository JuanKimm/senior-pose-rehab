...
public class ResultController {
    ...
    // 보호자 공유용 토큰 생성 (+ SMS 자동 발송)
    @PostMapping("/result/{resultId}/share-token")
    public ResponseEntity<?> createShareToken(
            @AuthenticationPrincipal String userId,
            @PathVariable Long resultId) {
        ...
        if (record.getUserId() == null || !record.getUserId().equals(Long.parseLong(userId))) {
            return ResponseEntity.status(HttpStatus.FORBIDDEN)
                    .body(Map.of("error", "본인의 운동 결과만 공유할 수 있습니다"));
        }

        notificationService.sendResultAlarm(resultId);
        return ResponseEntity.ok(Map.of("message", "공유 링크가 발송되었습니다"));
    }
